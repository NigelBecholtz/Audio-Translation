...
function updateProgress(data) {
    ...
    if (data.is_pending_approval) {
        ...
        // Update status badge for audio generation
        if (statusBadge) {
            statusBadge.className = 'inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-blue-100 text-blue-800 pulse-animation';
        }
        if (statusIcon) {
            statusIcon.className = 'fas fa-spinner fa-spin mr-2';
        }
        if (statusText) {
            statusText.textContent = '{{ __('Generating Audio...') }}';
        }
